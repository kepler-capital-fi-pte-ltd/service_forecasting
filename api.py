...
def get_data():
    if request.method == 'POST':
        try:
            ...
            await_result = subprocess.check_output(f"conda activate alphamethods | python {script}", shell=True)
            ...
            output = {}
            for i in methods:
                output.update({i : result[[i, "ds"]].rename(columns={i:"yhat"}).to_dict(orient="records")})
            return output
